...
export default function TransactionStatusPage() {
  ...
  const statusConfig: Record<Status, StatusConfig> = {
    PENDING: {
      ...
      title: t('pagePendingTitle'),
      desc: t('pagePendingDesc'),
      button: t('pagePendingBtn'),
      btnClass:
        'bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700',
    },
    PAID: {
      icon: <CheckCircle2 className="w-14 h-14 text-emerald-500" />,
      title: t('pagePaidTitle'),
      desc: t('pagePaidDesc'),
      button: t('pagePaidBtn'),
      btnClass: `
      bg-emerald-500 
  ...
  `,
    ...
    FAILED: {
      ...
      title: t('pageFailedTitle'),
      desc: t('pageFailedDesc'),
      button: t('pageFailedBtn'),
      btnClass: `
    bg-rose-500
 text-rose-600
      ...
      <div className="w-full max-w-md animate-fadeInUp">
        <div className="bg-white/80 dark:bg-white/10 backdrop-blur-lg rounded-3xl p-6 border border-gray-200 dark:border-white/10 shadow-xl space-y-6">
          ...
          <div className="space-y-3 text-sm">
            <Row label={t('pageRowOrderId')} value={data.order_id} />
            <Row label={t('pageRowPaymentNo')} value={data.payment_number} />
            <Row label={t('pageRowMethod')} value={data.payment_channel?.toUpperCase()} />

            {isPending && isQRIS && data.qr_code_url && <QRCodeSection qr={data.qr_code_url} />}

            <TotalRow amount={data.amount} label={t('pageTotal')} />
          </div>
...
function QRCodeSection({ qr }: { qr: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 pt-2">
      ...
      <p className="text-xs text-gray-500 text-center">{t('pageScanQr')}</p>
    </div>
  )
}

function TotalRow({ amount, label }: { amount: number; label: string }) {
  return (
